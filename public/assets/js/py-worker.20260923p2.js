...
const PYFILES=['data_processor.py','scientific_qc.py','calibration_utils.py','gap_filler.py','advanced_ml_gap_filler.py','external_sources.py','universal_import.py','phase2_qc.py','validation_engine.py','browser_engine.py'];
...
self.onmessage=async event=>{const m=event.data||{};try{await init();
 if(m.cmd==='inspect'){
   ...
   const csv=await pyodide.runPythonAsync(`browser_engine.export_csv(${m.compact?'True':'False'})`);send('exportReady',{text:csv,filename:m.compact?'fluxgapfill_phase2_results_compact.csv':'fluxgapfill_phase2_full_provenance.csv',mime:'text/csv'});
   ...
   const csv=await pyodide.runPythonAsync('browser_engine.qc_audit_csv()');send('exportReady',{text:csv,filename:'fluxgapfill_phase2_qc_audit.csv',mime:'text/csv'});
 }else if(m.cmd==='exportReport'){
   const text=await pyodide.runPythonAsync('browser_engine.validation_report_md()');send('exportReady',{text,filename:'fluxgapfill_phase2_validation_report.md',mime:'text/markdown'});
 }
