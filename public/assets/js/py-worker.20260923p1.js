...
const PYFILES=['data_processor.py','scientific_qc.py','calibration_utils.py','gap_filler.py','advanced_ml_gap_filler.py','external_sources.py','universal_import.py','browser_engine.py'];
...
self.onmessage=async event=>{const m=event.data||{};try{await init();
 if(m.cmd==='inspect'){
   ...
   const res=await callPy(`browser_engine.run_benchmark(${JSON.stringify(JSON.stringify(m.targets))}, ${JSON.stringify(JSON.stringify(m.durations))}, ${Number(m.windows||1)}, ${JSON.stringify(JSON.stringify(m.candidates))})`);send('benchmarkDone',{result:res});
 }else if(m.cmd==='fill'){
   const res=await callPy(`browser_engine.run_gap_fill(${JSON.stringify(JSON.stringify(m.targets))}, ${JSON.stringify(m.mode)}, ${JSON.stringify(m.fixedModel)}, ${Number(m.maxGapDays||90)})`);send('fillDone',{result:res});
 }else if(m.cmd==='export'){
   const csv=await pyodide.runPythonAsync(`browser_engine.export_csv(${m.compact?'True':'False'})`);send('exportReady',{csv,filename:m.compact?'fluxgapfill_results_compact.csv':'fluxgapfill_standardized_full.csv'});
 }else if(m.cmd==='exportBenchmark'){
   const csv=await pyodide.runPythonAsync('browser_engine.benchmark_csv()');send('exportReady',{csv,filename:'fluxgapfill_blocked_validation.csv'});
 }
